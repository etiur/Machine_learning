...
def fit():
    ...
    transformed_x_knn, test_x_knn, Y_train_knn, Y_test_knn, X_train_knn, X_test_knn = split_transform(X_knn, Y)
...
def predict(fitted, test_x_svc, test_x_knn, transformed_x_svc, transformed_x_knn):
...
def get_scores(Y_test_svc, Y_test_knn, Y_train_svc, Y_train_knn, test_pred, train_pred):
    ...
    knn_scores = print_score(Y_test_knn, test_pred.knn, train_pred.knn, Y_train_knn)
    ...
    ensemble1_score = print_score(Y_test_knn, vote_1_test, vote_1_train, Y_train_knn)
    ensemble1_purged = print_score(Y_test_knn, vote_1_test, vote_1_train, Y_train_knn, index1_test, index1_train, mode=1)
...
def writing(dataset1, te_report, tr_report, sheet_name, row=0):
    """Writes to excel"""
    if not path.exists(f"ensemble_scores/ensemble_new.xlsx"):
        wb = Workbook()
        ws = wb.active
        ws.title = sheet_name
        wb.save(filename=f"ensemble_scores/ensemble_new.xlsx")

    book = load_workbook('ensemble_scores/ensemble_new.xlsx')
    writer = pd.ExcelWriter('ensemble_scores/ensemble_new.xlsx', engine='openpyxl')
